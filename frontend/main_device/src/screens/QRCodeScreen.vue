...
const url = `http://${sessionStore.getIPAddress()}:${import.meta.env.VITE_PORT}/#/controller-lobby/${sessionStore.getSessionID()}`;
...
async function deleteGame() {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_IP_ADRESS_WITH_HTTP}:8080/game/`+id, {
        method: 'DELETE',
        headers: {
...
async function savePlayers() {
    try {
      ...
      for(const player of gameStore.players) {
        const response = await fetch(`${import.meta.env.VITE_IP_ADRESS_WITH_HTTP}:8080/player`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
